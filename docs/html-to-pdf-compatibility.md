...
| Page rules and breaks | Valid CSS Paged Media page-context properties are accepted; rendering currently applies common `size` values and a single-value `margin`, while other recognized declarations are retained as compatibility no-ops; malformed declarations, unknown properties, and invalid `size`, `margin*`, `page-orientation`, `marks`, or `bleed` values fail with `:invalid_css`; `@media print`, `@media only print`, and `@media all` rules participate in the print cascade while other media are skipped; `break-before`, `break-after`, `page-break-before`, `page-break-after` with `auto`, `page`, or `always`; `break-inside` and `page-break-inside` with `auto` or best-effort `avoid`; paragraphs fragment between complete visual lines                                                                                                                                                     |
