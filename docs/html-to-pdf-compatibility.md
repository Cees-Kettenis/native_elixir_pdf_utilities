...
| Text                  | `color`, `font-family`, `font-size`, `font-weight`, `font-style`, `line-height`, `text-align`, `text-transform`, `vertical-align`, `line-break`, `word-break`, `word-wrap`, `overflow-wrap`, `white-space`, `letter-spacing`; unitless and `normal` line heights retain their relative inheritance behavior while absolute line heights inherit as fixed lengths; hex colors, common named colors, `rgb()`, `rgba()`, `currentColor`, and transparent backgrounds and borders are accepted; alpha is ignored for partially transparent painted text and borders                                                                 |
